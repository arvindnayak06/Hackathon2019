...
public class TransactionEndpoint {
	...
	public Response saveTx(Transaction tx) {
		...
		logger.info("Inside saveTx method. invoking saveTxToDb method");
		try {
			boolean isSuccess = cqs.savetoTxDb(tx);
			logger.info("is success: "+isSuccess);
			if(isSuccess) {
				logger.info("save a success");
				response = Response.status(Response.Status.CREATED).type(MediaType.APPLICATION_JSON).entity("{\"status\":\"Success\"}").build();
			}else {
	...
	public Response updateTx(Transaction tx) {
		...
		logger.info("Inside updateTx method. invoking saveTxToDb method");
		try {
			boolean isSuccess = cqs.updateTxDb(tx);
			logger.info("is success: "+isSuccess);
			if(isSuccess) {
				logger.info("update a success");
				response = Response.status(Response.Status.OK).type(MediaType.APPLICATION_JSON).entity("{\"status\":\"Success\"}").build();
			}else {
	...
	public Response getTx(@QueryParam("token")String bankToken){
		...
		logger.info("Inside getTx method. invoking getFromTxDb method");
		try {
			tx = cqs.getFromTxDb(bankToken);
			if(tx != null) {
				...
				response = Response.status(Response.Status.NOT_FOUND).type(MediaType.APPLICATION_JSON).entity("{\"error\":\"No transaction found with bank token "+bankToken+"\"}").build();
			}
		}catch(MalformedURLException e) {
			logger.info("MalformedURLException");
			// can be thrown by cloudant client
