...
public class CloudantQueryService {
	...
	public boolean updateTxDb(Transaction tx) throws MalformedURLException{
		boolean isSuccess;

		connectToCloudant();
		db = connectToDb(client,"transaction_db", false);
		// save document to database
		logger.info("_id: "+tx.get_id() + " _rev: "+tx.get_rev());
		Response resp = db.update(tx);
		if(resp.getStatusCode()/100==2) {
			logger.info("Document save a success!!");
			isSuccess=true;
		}else {
	...
	public Transaction getFromTxDb(/*String mobileNo,*/String bankToken) throws MalformedURLException{
		...
		logger.info("bankToken: "+bankToken);
