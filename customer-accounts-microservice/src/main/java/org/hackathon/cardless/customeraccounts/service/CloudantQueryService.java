...
public class CloudantQueryService {
	...
	public boolean updateCustomerDetails(CustomerAccounts custAccs) throws MalformedURLException,DocumentConflictException{
		...
		logger.info("_id: "+custAccs.get_id() + " _rev: "+custAccs.get_rev());
		// custAccs.set_rev(null);
		Response resp = db.update(custAccs);
		logger.info("Response code from save: "+resp.getStatusCode());
		if(resp.getStatusCode()/100==2) {
			logger.info("Document update a success!!");
			isSuccess=true;
		}else {
			logger.info("Unable to update document: "+resp.getError());
			isSuccess=false;
		}
